...
void NatSpeakService::Connect(IntPtr serviceProviderPtr) {
    Connect(
        reinterpret_cast<::IServiceProvider*>(serviceProviderPtr.ToPointer())
    );
}
...
void NatSpeakService::InitializeIsrCentral(
    ::IServiceProvider* pServiceProvider
) {
    _piServiceProvider = pServiceProvider;

    ISrCentral^* ptr = ComHelper::QueryService<IDgnDictate^, ISrCentral^>(
        _piServiceProvider
    );
...
void NatSpeakService::InitializeSpeechServicesInterfaces() {
    // Speech Services
    IDgnSpeechServices^* ptr = ComHelper::QueryService<
        ISpchServices^, IDgnSpeechServices^>(_piServiceProvider);
    _idgnSpeechServices = (IDgnSpeechServices^) Marshal::GetObjectForIUnknown(
        IntPtr(ptr)
    );
...
bool NatSpeakService::IsDragonAlive() {
    ...
    IDgnSSvcAppTrackingNotifySink^ appTrackingSink = gcnew
    ...
    try {
        ...
        throw gcnew InvalidStateException(
            "Unexpected result when calling SSvcAppTrackingNotifySink::Register"
        );
    } catch (COMException^ e) {
...
void NatSpeakService::RegisterEngineSink() {
    ...
    pin_ptr<DWORD> key = &
        _key; // https://msdn.microsoft.com/en-us/library/1dz8byfh.aspx

    // Create an engine sink
    auto sink = gcnew SrNotifySink(
        gcnew Action<UInt64>(
            _grammarService,
            &NatSpeakInterop::GrammarService::PausedProcessor
        )
    );
    ...
    idgnSrEngineNotifySinkPtr = Marshal::GetIUnknownForObject(
        idgnSrEngineNotifySink
    );

    // Register our notification sinks
    // _isrCentral->Register(isrNotifySinkPtr, __uuidof(ISrNotifySink^), key);
    _isrCentral->Register(
        idgnSrEngineNotifySinkPtr,
        __uuidof(IDgnSrEngineNotifySink^),
        key
    );
